...
class _WalletScreenState extends State<WalletScreen>
  ...
  void _launchPaymentUrl(String url, int orderId) {
    // Открываем WebView для оплаты
    Navigator.push(
      context,
      MaterialPageRoute(
      ...
      // После закрытия WebView показываем диалог проверки статуса
      if (mounted) {
  ...
  void _showPaymentWaitingDialog(int orderId) {
    showDialog(
      ...
      builder: (context) => _PaymentWaitingDialog(
        orderId: orderId,
        onPaymentCompleted: () {
...
class _PaymentWaitingDialogState extends State<_PaymentWaitingDialog> {
  ...
  Future<void> _checkPaymentStatus() async {
    ...
    try {
      final token = Hive.box('user').get('auth_token', defaultValue: '') as String;
      final walletRepo = context.read<WalletBloc>().repository;
      
      final result = await walletRepo.checkPaymentStatus(
        userId: authState.user!.id,
        token: token,
        orderId: widget.orderId,
      );

      if (result['status'] == true && result['is_paid'] == true) {
        setState(() {
          _isChecking = false;
        ...
        // Закрываем диалог через 2 секунды после показа галочки
