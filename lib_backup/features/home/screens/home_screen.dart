...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    print('🔵 [HomeScreen] Listener added');
    _loadCachedCity();
  ...
  void _showAddStoryDialog(BuildContext context) {
    ...
    final dialogBgColor = isDark ? const Color(0xff233040) : Colors.white;
              ...
              children: [
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: isDark ? const Color(0xff151e27) : const Color(0xffF0EEFF),
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    if (video != null) {
                      if (context.mounted) {
                        _showStorySettingsDialog(context, filePath: video.path, isPhoto: false);
                      }
                    }
                  },
                  style: ElevatedButton.styleFrom(
              ...
              categoriesApi.getCategories().then((result) {
                setBottomSheetState(() {
                  ...
                  if (result['status'] == true && result['data'] != null) {
                    final data = result['data'];
                    print('🔵 [CategoryOptions] Data type: ${data.runtimeType}, Data: $data');
                    if (data is List) {
                      categories = data;
                      filteredCategories = categories;
                      print('✅ [CategoryOptions] Loaded ${categories.length} categories (direct list)');
                    } else if (data is Map && data.containsKey('data') && data['data'] != null) {
                      categories = data['data'] as List;
                      filteredCategories = categories;
                      print('✅ [CategoryOptions] Loaded ${categories.length} categories (from map)');
                    }
                  }
                });
              });
              ...
              child: Column(
                children: [
                  TextField(
                    ...
                    style: GoogleFonts.montserrat(color: textColor),
                    decoration: InputDecoration(
                      hintText: 'Укажите название категории',
                      hintStyle: GoogleFonts.montserrat(
                        color: isDark ? Colors.white54 : Colors.grey[400],
                      ),
                      filled: true,
      ...
      body: Stack(
        children: [
          RefreshIndicator(
            ...
            // Тут начинается логика отображения скрола
                                      ...
                                      children: [
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            border: Border.all(color: const Color(0xff917dfa), width: 1.5),
                                          ),
                                          child: const Icon(Icons.arrow_forward, color: Color(0xff917dfa), size: 32),
                                        ),
                                        const SizedBox(height: 4),
                                        Text('Все категории', style: GoogleFonts.montserrat(fontSize: 12), textAlign: TextAlign.center, maxLines: 2),
                                      ],
                                    ),
                                  ),
                                );
                          ...
                          child: Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('Не удалось загрузить категории', style: GoogleFonts.montserrat(fontSize: 12, color: Colors.grey)),
                              ],
                            ),
                          ),
                        );
            ...
            // Заканчивается логика отображения скрола
