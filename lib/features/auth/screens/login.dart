...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    ...
    _deepLinkService.onOAuthSuccess = (token, userId) {
      print('✅ [14] onOAuthSuccess вызван: token=$token, userId=$userId');
    ...
    _deepLinkService.onOAuthError = (error) {
      print('❌ [15] onOAuthError вызван: $error');
  ...
  void dispose() {
    ...
    // ✅ ОЧИЩАЕМ CALLBACK
    _deepLinkService.onOAuthSuccess = null;
    _deepLinkService.onOAuthError = null;
  ...
  Future<void> _onYandexLogin() async {
    try {
      _oauthService.loginWithYandex(context);
      showSwipeDownNotification(
        context,
        message: 'Авторизация через Яндекс',
      );
    } catch (e) {
      if (mounted) {
        showSwipeDownNotification(
  ...
  Future<void> _onVKLogin() async {
    try {
      _oauthService.loginWithVK(context);
      // Если дошли сюда — браузер открыт успешно
      showSwipeDownNotification(
        context,
