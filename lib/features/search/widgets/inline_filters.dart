//G:\hashtagg_app\lib\features\search\widgets\inline_filters.dart
...
class InlineFilters extends StatefulWidget {
  ...
  final Function(String filterId, String itemId, bool isSelected)
  onFilterChanged;
...
class _InlineFiltersState extends State<InlineFilters> {
  ...
  Widget build(BuildContext context) {
    ...
    final filterOptions =
        widget.filterData!['options'] as Map<String, dynamic>?;
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Switch(
                                ...
                                onChanged: (v) =>
                                    widget.onOptionChanged(entry.key, v),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          isExpanded: true,
          items: (filter['items'] as List).map<DropdownMenuItem<String>>((
            item,
          ) {
  ...
  Widget _buildCheckboxFilter(
    ...
    print(
      '🔵 [InlineFilters] _buildCheckboxFilter for ${filter['name']}, items count: ${items?.length ?? 0}',
    );
    ...
    return Wrap(
      ...
      children: items.map<Widget>((item) {
        ...
        final isSelected =
            widget.selectedFilters[filterId]?.contains(itemId) ?? false;

        print(
          '🔵 [InlineFilters] Item: ${item['name']}, id: $itemId, selected: $isSelected',
        );
  ...
  Widget _buildInputFilter(
    ...
    return TextField(
      controller: TextEditingController(
        text:
            widget.selectedFilters[filter['id'].toString()]?.firstOrNull ?? '',
      ),
      onChanged: (value) {
        widget.onFilterChanged(filter['id'].toString(), value, false);
