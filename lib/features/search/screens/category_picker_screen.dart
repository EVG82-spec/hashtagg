...
class _CategoryPickerScreenState extends State<CategoryPickerScreen> {
  ...
  void _onSearchChanged(String query) async {
    ...
    try {
      ...
      print('🔍 [CategoryPicker] Search result: $result'); // ← лог
      ...
      print('🔴 [CategoryPicker] Search error: $e');
      setState(() {
        _searchResults = [];
        _isSearching = false;
      });
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            _searchQuery.isNotEmpty ? 'Поиск' : _currentTitle,
  ...
  Widget _buildSearchResults() {
    ...
    print('🔍 _searchResults в UI: ${_searchResults.length}');
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final parent = item['parent'] ?? 0;
        final chain = parent != 0 ? 'ID категории: $parent' : '';
  ...
  void _selectSearchResult(int categoryId, String name) {
    // Возвращаем выбранную категорию
    if (widget.returnId) {
      Navigator.pop(context, {'id': categoryId, 'name': name});
    } else {
