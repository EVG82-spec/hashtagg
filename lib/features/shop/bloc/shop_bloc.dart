...
class ShopBloc extends Bloc<ShopEvent, ShopState> {
  ...
  Future<void> _onLoadShopForEdit(
      LoadShopForEdit event,
    ...
    try {
      ...
      if (response['status'] == true && response['data'] != null) {
        ...
        if (convertedData['id_user'] is String) {
          convertedData['id_user'] = int.tryParse(convertedData['id_user'] as String) ?? 0;
          print('   id_user converted: ${convertedData['id_user']}');
        }

        print('✅ [ShopBloc] Converting status...');
        if (convertedData['status'] is String) {
          convertedData['status'] = int.tryParse(convertedData['status'] as String) ?? 0;
        ...
        if (convertedData['id_theme_category'] is String) {
          convertedData['id_theme_category'] = int.tryParse(convertedData['id_theme_category'] as String) ?? 0;
          print('   id_theme_category converted: ${convertedData['id_theme_category']}');
        }

        print('✅ [ShopBloc] Converting id...');
        if (convertedData['id'] is String) {
          convertedData['id'] = int.tryParse(convertedData['id'] as String) ?? 0;
