...
class _MapMiniatureState extends State<MapMiniature> {
  ...
  Future<void> _loadListings() async {
    print('🟣 [1] _loadListings() START');
    setState(() => _isLoading = true);

    try {
      print('🟣 [2] cityId: ${widget.filters.cityId}');
      print('🟣 [3] categoryId: ${widget.filters.categoryId}');
      ...
      print('🟣 [4] result.success: ${result.success}');
      print('🟣 [5] result.data: ${result.data != null ? 'not null' : 'null'}');
      ...
      print('🟣 [6] allAds count: ${allAds.length}');
      ...
      print('🟣 [7] adsWithCoords count: ${adsWithCoords.length}');
      _listings = adsWithCoords;

      setState(() => _isLoading = false);

      if (_mapController != null) {
        print('🟣 [8] mapController exists, building placemarks...');
        await _buildPlacemarks();
      } else {
        print('🟣 [8] mapController is NULL!');
      }
    } catch (e) {
  ...
  Future<void> _buildPlacemarks() async {
    print('🟣 [13] _buildPlacemarks() START');
    try {
      final listings = _listings;
      if (listings.isEmpty) {
        print('🟣 [14] NO listings to display');
        return;
      }

      print('🟣 [15] building ${listings.length} placemarks...');
      ...
      if (mounted) {
        ...
        print('🟣 [16] placemarks added to map');
      }
    } catch (e, st) {
      print('🟣 [ERROR] _buildPlacemarks: $e\n$st');
    }
  }
            ...
            child: YandexMap(
              ...
              onMapCreated: (controller) async {
                ...
                if (widget.filters.cityLat != null &&
                  ...
                  zoom = 12.0;
                  print('🟣 [10] city coordinates: ${widget.filters.cityLat}, ${widget.filters.cityLon}');
                } else {
                  targetPoint = _defaultTarget;
                  zoom = _defaultZoom;
                  print('🟣 [10] default coordinates (Moscow)');
                }
                ...
                print('🟣 [11] camera moved');

                if (_listings.isNotEmpty) {
                  print('🟣 [12] listings not empty, building placemarks...');
                  await _buildPlacemarks();
                } else {
                  print('🟣 [12] listings EMPTY!');
                }
              },
              onMapTap: (point) {
                widget.onTap();
