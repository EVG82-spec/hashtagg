...
class AdsApiRepository {
  ...
  Future<Map<String, dynamic>> uploadPhoto({
    ...
    try {
      ...
      print('🔵 [AdsApi] userId: $userId');
      print('🔵 [AdsApi] token length: ${token.length}');
      print('🔵 [AdsApi] imageBase64 length: ${imageBase64.length}');
      ...
      final response = await _dio.post(
        ...
        options: Options(
          validateStatus: (status) => true, // ← НЕ ВЫБРАСЫВАТЬ ИСКЛЮЧЕНИЕ
        ),
      );

      print('🔵 [AdsApi] Upload photo response status: ${response.statusCode}');
      print('📥 [AdsApi] Upload photo response: ${response.data}');
      ...
      if (response.statusCode == 200) {
        ...
        // ✅ ПРОВЕРЯЕМ НАЛИЧИЕ ДАННЫХ (а не status)
        if (data['data'] != null && data['data']['name'] != null) {
          ...
          print('🔴 [AdsApi] Server error: ${data['error'] ?? 'No data'}');
          return {
            'status': false,
        ...
        print('🔴 [AdsApi] HTTP error: ${response.statusCode}');
        return {
          'status': false,
  ...
  Future<Map<String, dynamic>> createAd({
    ...
    try {
      ...
      print('🔵 [AdsApi] userId: $userId');
      print('🔵 [AdsApi] adData: $adData');

      final response = await _dio.post(
        '/systems/api/controller.php',  // ← ВОЗВРАЩАЕМСЯ К controller.php
        queryParameters: {
          'key': ApiConfig.apiKey,
          'route': 'create_ad/create',  // ← ПРАВИЛЬНЫЙ РОУТЕР
        },
        data: {
          'id_user': userId,
          'token': token,
      ...
      print('🔵 [AdsApi] Create ad response body: ${response.data}');
      print('🔵 [AdsApi] Create ad response headers: ${response.headers}');

      print('🔵 [AdsApi] Create ad response status: ${response.statusCode}');
      print('📥 [AdsApi] Create ad response: ${response.data}');
