...
class NotificationBloc extends Bloc<NotificationEvent, NotificationState> {
  ...
  Future<void> _registerPushToken(String userId, String authToken) async {
    try {
      print('📱 [NotificationBloc] Registering push token for user $userId');
      final success = await PushTokenService().registerToken(userId, authToken);
      if (success) {
        print('✅ [NotificationBloc] Push token registered successfully');
      } else {
        print('⚠️ [NotificationBloc] Push token registration failed');
      }
    } catch (e) {
      print('❌ [NotificationBloc] Error registering push token: $e');
    }
  }
  ...
  Future<void> _deletePushToken(String userId, String authToken) async {
    ...
    try {
      print('📱 [NotificationBloc] Deleting push token for user $userId');
      final success = await PushTokenService().deleteToken(userId, authToken);
      if (success) {
        print('✅ [NotificationBloc] Push token deleted successfully');
      } else {
        print('⚠️ [NotificationBloc] Push token deletion failed');
      }
    } catch (e) {
      print('❌ [NotificationBloc] Error deleting push token: $e');
    }
  }
