...
class PushTokenService {
  ...
  // Сохраняем токен локально, чтобы не спамить регистрацией
  String? _cachedToken;

  /// Получить текущий push-токен (заглушка, пока нет реального VK SDK)
  Future<String?> getDeviceToken() async {
    try {
      // ⚠️ ПОКА ЗАГЛУШКА: используем уникальный ID устройства
      // В будущем замените на реальный токен от VK Push SDK
      final prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString('push_device_token');
      if (token == null) {
        // Генерируем уникальный ID на основе данных устройства
        String deviceId = await _getDeviceId();
        token = 'vk_push_${deviceId}_${DateTime.now().millisecondsSinceEpoch}';
        await prefs.setString('push_device_token', token);
      }
      _cachedToken = token;
      return token;
    } catch (e) {
      print('❌ Ошибка получения push-токена: $e');
      return null;
    }
  }
  ...
  Future<String> getDeviceType() async {
    try {
      ...
      print('❌ Ошибка определения типа устройства: $e');
      return 'other';
    }
  }

  /// Регистрация токена на сервере
  Future<bool> registerToken(String userId, String authToken) async {
    try {
      ...
      if (response.statusCode == 200) {
        print('✅ Push-токен успешно зарегистрирован');
        return true;
      } else {
        print('❌ Ошибка регистрации токена: ${response.statusCode}');
        return false;
      }
    } catch (e) {
      print('❌ Ошибка при регистрации токена: $e');
      return false;
    }
  }

  /// Удаление токена при выходе из аккаунта
  Future<bool> deleteToken(String userId, String authToken) async {
    try {
      String? token = await getDeviceToken();
      ...
      if (response.statusCode == 200) {
        print('✅ Push-токен удалён');
        final prefs = await SharedPreferences.getInstance();
        await prefs.remove('push_device_token');
        return true;
      } else {
        print('❌ Ошибка удаления токена: ${response.statusCode}');
        return false;
      }
    } catch (e) {
  ...
  // Вспомогательный метод для получения уникального ID устройства
